/*
 ...
 Date	     : August 23, 2016
 Version	 : 1.0
 Description : This program will calculate a measure, given an input file
 	 	 	 	 of maximal cliques (generated by mce.cu)
 ...
 * mcMeasure.cu
 *
 * This program will calculate a measure based on a set of maximal cliques.
 * The measure used is the one provided in:
 * 	"Maximal Clique Enumeration in Finding Near Neighbourhoods"
 * 	by Christopher J. Henry and Sheela Ramanna
 *
 *
 *
 ...
 *			Allows the user to specify a file other than cliques.txt
 *			to be used for input
 *		-sc: Specifies that cliques consisting of only one graph node
 *				should not factor into the equation
 *
 * [out]:
 *		The final measure calcualted
 *
 * [return]:
 * 		Void
 *
...
int main(int argc, char **argv)
{
	...
	std::string inFile = "cliques.txt";

	//Total vertices in adjacency matrix
	unsigned v = 0;

	//Factor single cliques into the measure?
	bool singleCliques = false;
	...
	if(inputFile.fail()){
		std::cerr << "Error: Maximal Clique file, " << inFile.c_str() << ", could not be found." << std::endl;
		exit(1);
	}else{
		std::cout << "Reading from " << inFile.c_str() << "." << std::endl;
	}

	//Counts of elements in each line
	// xCount + yCount = cardinality
	unsigned xCount, yCount, cardinality;
	//Holds the final total of the measure
	double total = 0;
	//Gives the split point between the two sets
	//ie, given two sets of 50 elements, 0-49 is set 1, 50-99 is set 2
	//Thus, < v is set 1, >= v is set 2
	v = v / 2;

	//Holds integer value read in from file
	unsigned temp;
	//Total sum of all clique cardinalities
	unsigned totalCardinality = 0;
	//Current line being read in
	std::string curLine;


	//Read in every line of input file
	while(getline(inputFile, curLine)){
		...
		//Read in the values from this line
		while(line >> temp){
			if(temp < v){
				xCount++;
			}else{
		...
		//std::cout << "Clique size: " << cardinality << " xCount: " << xCount << " yCount: " << yCount << " ! " << cardinality * ((double)min(xCount, yCount) / max(xCount, yCount)) << "\n";

			//Update the total
		if(cardinality > 1 || singleCliques == true){
			total += cardinality * ((double)min(xCount, yCount) / max(xCount, yCount));
		}else{
			//If the user doesn't want single cliques counted, then decrement the totalCardinality to account for this
			totalCardinality--;
		}
	}

	//Apply weight factor to the total measure and output it

	total = powf(totalCardinality, -1) * total;
	//total = 1 - powf(totalCardinality, -1) * total;
	std::cout << "The final measure is: " << total << "\n";
